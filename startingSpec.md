...
### Шаг 6. Блок про детей: только факт наличия детей
...
Поведение:
- при выборе `Да` сохранить `{{has_children}} = true` и перейти к шагу 7;
- при выборе `Нет` сохранить `{{has_children}} = false`, установить `{{children_study_quran}} = null` и перейти к шагу 8;
...
### Шаг 7. Вопрос по детям: изучают ли Коран
...
Поведение:
- при выборе `Да` сохранить `{{children_study_quran}} = true`, переход к шагу 8;
- при выборе `Нет` сохранить `{{children_study_quran}} = false`, переход к шагу 8;
- если пользователь вводит текст вместо кнопки, допускается `да/нет` (без учёта регистра);
- при любом другом вводе повторно показать кнопки.
...
### Шаг 9. Сегментация по уровню чтения Корана
Отправить вопрос:
...
### Шаг 10. Завершение регистрации и выдача ссылки на канал курса
...
- заполнить в записи:
  - `user_id`, `telegram_first_name`, `telegram_username`, `user_name`, `age`, `has_children`, `children_study_quran`, `phone`, `reading_level`;
  - `consent_given = true`;
...
### Шаг 11. Автоматический переход в следующий сценарий
- через 3 минуты после шага 10 автоматически проверить подписку пользователя на канал курса;
- если подписка подтверждена, отправить пользователю сообщение Flow 2 (`Продолжение реф`);
- если подписка не подтверждена, продолжать фоновую проверку (membership update и/или периодический re-check) и отправить Flow 2 сразу после подтверждения;
- отдельная кнопка `Проверить подписку` не используется.

### Шаг 12. Передача в следующий сценарий
- действие: запуск автоматизации Flow 2 (`Продолжение реф`).

## 5) Спецификация данных

### 5.1 Карточка пользователя / запись регистрации
Запись пользователя создаётся один раз после завершения анкеты (шаг 10), когда уже собраны имя, возраст, данные по детям, телефон и уровень чтения.
...
- `has_children` (bool, ответ шага 6);
- `children_study_quran` (bool, ответ шага 7; `null` при `has_children = false`);
- `phone` (номер из шага 8);
...
- `reading_level` (одно из 5 значений шага 9);
...
- `{{age}}` — возраст из шага 5;
- `{{has_children}}` — ответ `Да/Нет` из шага 6;
- `{{children_study_quran}}` — ответ `Да/Нет` из шага 7;
- `{{phone}}` — телефон из шага 8.
...
- Пользователь не подписан на канал курса:
  подписка проверяется автоматически по membership update или периодическим re-check.
- Проверка подписки на канал курса временно недоступна:
  пользователь не блокируется, фоновая перепроверка выполняется в следующих тиках.
...
- Пользователь отправил некорректный ответ на вопрос `Изучают ли они Коран?`:
  повторно показывать кнопки `Да` / `Нет` на шаге 7.
- Пользователь отправил текст вместо контакта:
  повторно запрашивать номер телефона на шаге 8.
- Пользователь отправил чужой контакт (`contact.user_id != user_id`):
  отклонять контакт и повторять шаг 8.
...
4. Бот выдаёт ссылку на канал курса.
5. Через 3 минуты автоматически отправляется реферальная программа (Flow 2).
